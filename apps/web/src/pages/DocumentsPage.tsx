...
const DOCUMENT_TYPES: Record<string, { icon: React.ElementType; color: string; gradient: string; bgColor: string; borderColor: string; label: string }> = {
  ...
  storyboard: { icon: FileImage, color: '#f59e0b', gradient: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', bgColor: 'rgba(245, 158, 11, 0.12)', borderColor: 'rgba(245, 158, 11, 0.25)', label: '故事�? },
...
const STATUS_CONFIG: Record<string, { color: string; bgColor: string; borderColor: string; label: string }> = {
  ...
  pending: { color: '#f59e0b', bgColor: 'rgba(245, 158, 11, 0.12)', borderColor: 'rgba(245, 158, 11, 0.2)', label: '待处�? },
  in_progress: { color: '#3b82f6', bgColor: 'rgba(59, 130, 246, 0.12)', borderColor: 'rgba(59, 130, 246, 0.2)', label: '进行�? },
  completed: { color: '#10b981', bgColor: 'rgba(16, 185, 129, 0.12)', borderColor: 'rgba(16, 185, 129, 0.2)', label: '已完�? },
  published: { color: 'colors.accent', bgColor: 'colors.accentBg', borderColor: 'rgba(139, 92, 246, 0.2)', label: '已发�? },
};
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: 'colors.textMuted', fontSize: '14px' }}>加载�?..</p>
        </div>
      </div>
    );
  }
      ...
      <div style={{
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '20px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
              ...
              <div>
                ...
                <p style={{ fontSize: '13px', color: 'colors.textMuted', margin: 0 }}>�?{documents.length} 个文�?/p>
              </div>
            </div>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            >
              <option value="newest">最新创�?/option>
              <option value="oldest">最早创�?/option>
              <option value="updated">最近更�?/option>
              <option value="title">按标�?/option>
            </select>
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px' }}>
        ...
        {filteredAndSortedDocuments.length === 0 ? (
          <div style={{
            ...
            <p style={{ fontSize: '14px', color: 'colors.textMuted', marginBottom: '24px' }}>
              {searchQuery ? '尝试调整搜索条件或筛选器' : '点击右上角创建您的第一个文�?}
            </p>
            {!searchQuery && (
        ...
        {filteredAndSortedDocuments.length > 0 && (
          <div style={{
            ...
            <span style={{ fontSize: '14px', color: 'colors.textMuted' }}>
              显示 {filteredAndSortedDocuments.length} 个文�?              {selectedType && ` · 类型: ${getDocumentTypeConfig(selectedType).label}`}
            </span>
          </div>
        )}
      </div>
