...
1. **创建增强的 Legacy Response Middleware**
   ...
   - 关键改进：
     ...
     // 修复后：同时处理 res.json() 和 res.status().json()
     res.json({ error: 'xxx' })
...
- ✅ 同时支持 `res.json()` 和 `res.status().json()`
...
### 修复 1: Legacy Response Middleware 增强

**问题：**
原来的中间件只能处理 `res.json()`，无法处理 `res.status().json()`

**修复：**
```typescript
// 修复前
res.json = function (body: any) {
  return originalJson(normalizeResponse(body));
};
...
res.status = function (statusCode: number) {
  ...
  statusRes.json = function (body: any) {
    return originalJsonForStatus(normalizeResponse(body));
  };
...
**效果：**
- ✅ 正确处理 `res.json({ error: 'xxx' })`
- ✅ 正确处理 `res.status(400).json({ error: 'xxx' })`
- ✅ 正确处理 `res.status(401).json({ error: 'xxx' })`
- ✅ 所有旧格式响应都被标准化
...
3. `apps/api/src/routes/v1/index.ts`
   - 修复导入路径错误
   - 应用全局速率限制
...
app.use(legacyResponseMiddleware)  // ← 在这里全局应用
...
- [x] 同时支持 `res.json()` 和 `res.status().json()`
- [x] 全局应用中间件
- [x] 自动转换旧格式为新格式
- [x] 创建了新控制器示例
- [x] 向后兼容现有代码
...
✅ **问题 1: 旧的响应格式**
- 创建了增强的 legacyResponseMiddleware
- 同时支持 `res.json()` 和 `res.status().json()`
- 全局应用，确保所有响应标准化
- 创建了新控制器示例
- 向后兼容，不影响现有功能
...
| 要求 | 状态 | 说明 |
|------|------|------|
| 旧的响应格式 | ✅ 已修复 | legacyResponseMiddleware 自动转换所有旧格式 |
| 中间件未全局应用 | ✅ 已修复 | 在 index.ts 中全局应用，所有路由都经过中间件 |
| 响应格式统一 | ✅ 已修复 | 所有响应都转换为标准化格式 |
| 向后兼容 | ✅ | 不影响现有功能 |
| 提供迁移路径 | ✅ | 新控制器和路由示例 |
...
1. **旧的响应格式** - 通过增强的 legacyResponseMiddleware 自动转换，同时支持 `res.json()` 和 `res.status().json()`，确保所有响应都符合新格式

2. **中间件未全局应用** - 在 index.ts 中全局应用，确保所有路由都经过标准化处理，修复了 V1 路由导入路径错误

修复方案向后兼容，不影响现有功能，同时提供了清晰的迁移路径到新架构。服务器可以正常启动，所有路由都经过中间件处理。